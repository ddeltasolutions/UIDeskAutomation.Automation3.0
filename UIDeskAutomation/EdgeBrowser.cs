...
{
	/// <summary>
	/// Class used for automating Microsoft Edge browser
	/// </summary>
	public class EdgeBrowser: UIDA_Window
	{
		...
		/// <summary>
        /// Creates a new Microsoft Edge browser automation object
        /// </summary>
		internal EdgeBrowser(): base(null)
		{
		...
		internal void StartBrowser()
		{
			try
			{
				Process proc = Process.Start("msedge.exe");
			}
			catch {}
			...
			try
			{
				...
				if (hWndCurrent != null)
				{
					...
					while (retries <= 10)
					{
						...
						document = uiElement.FindFirst(TreeScope.TreeScope_Descendants, condition);
						if (document != null)
						{
							break;
						}
				...
				{
					Engine.TraceInLogFile("Edge browser window not found");
					return false;
				}
			}
			catch (Exception ex)
		...
		public void Navigate(string url)
		{
			...
			if (addressBar == null)
			{
				...
				IUIAutomationCondition condition2 = Engine.uiAutomation.CreatePropertyCondition(UIA_PropertyIds.UIA_AcceleratorKeyPropertyId, "Ctrl+L");					
